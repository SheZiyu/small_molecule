...
file_path = '/home/ziyu/PycharmProjects/pythonProject/small_sys_gnn/output_images_debug_forward/1a10A/all_positions.npy'
# Load the numpy array from the .npy file
xyz = np.load(file_path)
print(f"Loaded positions array shape: {xyz.shape}")

# Load the PDB file to get the topology
pdb_file = '/home/ziyu/PycharmProjects/pythonProject/small_sys_gnn/output_images_debug_forward/1a10A/1a10A/final_structure_no_water.pdb'
...
output_dir = '/home/ziyu/PycharmProjects/pythonProject/small_sys_gnn/output_images_debug_forward/1a10A'
os.makedirs(output_dir, exist_ok=True)
print(f"Output directory: {output_dir}")

with mda.Writer(os.path.join(output_dir, 'forward_trajectory.pdb'), new_universe.atoms.n_atoms) as PDB_writer:
    PDB_writer.write(new_universe.atoms)

with mda.Writer(os.path.join(output_dir, 'forward_trajectory.dcd'), new_universe.atoms.n_atoms, unitcell=unitcell) as DCD_writer:
    for ts in new_universe.trajectory:
        DCD_writer.write(new_universe.atoms)

with mda.Writer(os.path.join(output_dir, 'forward_trajectory.xtc'), new_universe.atoms.n_atoms, unitcell=unitcell) as XTC_writer:
    for ts in new_universe.trajectory:
