...
from utils.auxiliary import calculate_rmsf
...
def process_folder(folder, folder_path):
    ...
    xtc_files = [
        file for file in os.listdir(folder_path) if file == "output_trajectory.xtc"
    ]
    if len(pdb_files) != 1 or len(xtc_files) != 1:
        print(
            f"Folder {folder_path} does not contain exactly one PDB and one XTC file."
        )
        return
...
if __name__ == "__main__":
    ...
    config = parse_toml_file(
        "/home/ziyu/PycharmProjects/pythonProject/small_sys_gnn/config.toml"
    )
    directory_path = (
        "/home/ziyu/PycharmProjects/pythonProject/small_sys_gnn/small_sys/sys_test"
    )
    ...
    with torch.no_grad():
        for idx, data in enumerate(test_loader):
            if idx % num_frames_to_process == 0:
                ...
                pred_rmsf = rmsf_results.get(
                    name
                )  # Get predicted RMSF or None if not found
