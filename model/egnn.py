...
class EGNN_Sparse_Network(nn.Module):
    ...
    def __init__(self, n_layers, feats_dim,
                 ...
                 global_linear_attn_dim_head=512,
...
class DynamicsEGNN(nn.Module):
    def __init__(self, n_layers, feats_dim,
        ...
        self.time_embedding = FourierTimeEmbedding(embed_dim=feats_dim, input_dim=1)
        self.mlp = MLP([feats_dim-1, feats_dim], iscond=False)
        self.model = model(n_layers, feats_dim)

    def forward(self, t, x, edge_index, batch, edge_attr):
        t = self.mlp(self.time_embedding(t))
        x[:, 3:] = t + x[:, 3:]
        x = self.model(x, edge_index, batch, edge_attr)
        return x