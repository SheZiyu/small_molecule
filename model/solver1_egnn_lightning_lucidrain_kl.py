...
class DDPM(nn.Module):
    ...
    def reverse_ode(self, x_T, h, noise_net, M=30, return_all=False, **kwargs):
        """
        Performs the reverse denoising process, reconstructing the data from its noised state.
        ...
        for i in range(len(t)):
            x = noise_net(x=x, t=t[i][None], **kwargs) # Apply the reverse denoising solver.
            # print(x)
            xs.append(x.clone()[:, :3])
        if return_all:
    ...
    def reverse_denoise(self, x_T, h, noise_net, solver, order=1, M=30, return_all=False, **kwargs):
        """
        Performs the reverse denoising process, reconstructing the data from its noised state.
        ...
        for i in range(1, len(t)//order):
            x = solver(x, h, t[i - 1], t[i], noise_net, **kwargs) # Apply the reverse denoising solver.
            xs.append(x.clone())
        if return_all:
            return torch.stack(xs)
        else:
    ...
    def adaptive_reverse_denoise(self, x_T, h, noise_net,
        ...
        if order3 > 0:
            for i in range(1, order3+1):
                x = solver3(x, h, t[i - 1], t[i], noise_net, **kwargs) # Apply the reverse denoising solver.
                xs.append(x.clone())
        if order2 > 0:
            for j in range(1, order2+1):
                x = solver2(x, h, t[j - 1], t[j], noise_net, **kwargs)  # Apply the reverse denoising solver.
                xs.append(x.clone())
        if order1 > 0:
            for k in range(1, order1+1):
                x = solver1(x, h, t[k - 1], t[k], noise_net, **kwargs)  # Apply the reverse denoising solver.
                xs.append(x.clone())
        if return_all:
            return torch.stack(xs)
        else:
    ...
    def solver1(self, x, h, t_im1, t_i, noise_net, **kwargs):
        """
        ...
        return self.alpha_t(t_i) / self.alpha_t(t_im1) * x - \
            self.sigma_t(t_i) * torch.expm1(h_i) * noise_net(x=torch.cat([x, h], dim=-1), t=t_im1[None], **kwargs)[:, :3]

    def solver2(self, x, h, t_im1, t_i, noise_net, r1=0.5, **kwargs):
        """
        A solver function for the reverse denoising process, computing the next state.
        ...
        model_im1 = noise_net(x=torch.cat([x, h], dim=-1), t=t_im1[None], **kwargs)[:, :3]

        x_s1 = (alpha_s1 / alpha_tim1 * x
                - (sigma_s1 * phi_11) * model_im1)
        model_s1 = noise_net(x=torch.cat([x_s1, h], dim=-1), t=s1[None], **kwargs)[:, :3]
    ...
    def solver3(self, x, h, t_im1, t_i, noise_net, r1=1./3., r2=2./3., **kwargs):
        """
        A solver function for the reverse denoising process, computing the next state.
        ...
        model_s2 = noise_net(x=torch.cat([x_s2, h], dim=-1), t=s2[None], **kwargs)[:, :3]
...
class LitModel(L.LightningModule):
    def __init__(self, config):
        ...
        self.model = DynamicsEGNN(1, config['node_dim']) #6.8k
        self.dpm = DDPM()
    ...
    def training_step(self, batch):
        ...
        x = self.model(
            t=t_assigned,
            x=torch.cat([noised_pos, source_graphs.x], dim=-1),
            edge_index=source_graphs.edge_index,
            batch=source_graphs.batch,
            edge_attr=source_graphs.edge_attr
        )

        transformed_graphs = x[:, 0:3]
        # loss from one frame to another frame
        loss = F.kl_div(torch.log(transformed_graphs + 1e-10), target_graphs.pos, reduction='batchmean',
                        log_target=False)
    ...
    def transform_frame(self, batch, lmd, is_noise=False):
        ...
        with torch.no_grad():
            ...
            lmd = torch.full((1, 1), lmd,
                             device=source_graphs.pos.device)  # or any specific lambda you want to use during testing
            t = self.dpm.t_lambda(lmd)
...
if __name__ == "__main__":
    ...
    # model = LitModel.load_from_checkpoint('/home/ziyu/PycharmProjects/pythonProject/small_sys_gnn/output/solver1_egnn_test_beta_8_1-v10.ckpt', config=config)
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename='/home/ziyu/PycharmProjects/pythonProject/small_sys_gnn/output/solver1_egnn_test_beta_8_1',
        monitor='val_loss',
        mode='min'
    )
