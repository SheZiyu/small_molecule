...
class DataAnalysis:
    ...
    def preprocess_coordinate_onehot(self):
        ...
        for zip_file_name in os.listdir(self.directory_path):
            if zip_file_name.endswith(".zip"):
                ...
                xtc_files = [
                    file for file in os.listdir(temp_dir_path) if file.endswith(".xtc")
                ]
                if not xtc_files:
                    print("No XTC files found in {}".format(temp_dir_path))
                pdb_file = [
                    file for file in os.listdir(temp_dir_path) if file.endswith(".pdb")
                ]
                if not pdb_file:
                ...
                for xtc_file in xtc_files:
                    ...
                    try:
                        ...
                        # rmsd_analysis.run()
                        one_hot_encoded_atoms = self._one_hot_encode_atoms(
                            u, selection=self.selection
                        )
                    ...
                    coordinates_array = np.empty(
                        (
                            len(u.trajectory[0 : self.num_frames_to_process]),
                            u.select_atoms(self.selection).n_atoms,
                            3,
                        )
                    )
                    # Iterate through the trajectory frames and store coordinates
                    for i, ts in enumerate(
                        u.trajectory[0 : self.num_frames_to_process]
                    ):
        ...
        with h5py.File(self.h5_filename, "w") as hf:
            ...
            for i, (coordinates_array, one_hot_traj, atoms_num, name) in enumerate(
                zip(
                    self.coordinates_arrays,
                    self.one_hot_trajs,
                    self.atoms_nums,
                ...
                group.create_dataset(
                    "coordinates_array_{}".format(i), data=coordinates_array
                )
    ...
    def _one_hot_encode_residues(self, u, selection):
        ...
        selected_residues = u.select_atoms(
            selection
        ).residues  # Select residues based on the given selection
        #
        ...
        residue_to_index = {
            resname: i for i, resname in enumerate(unique_residue_names)
        }
    ...
    def load_data_from_hdf5(self, file=None, group_name="h5"):
        ...
        with h5py.File(file, "r") as hf:
            ...
            for i in range(
                len(group.keys()) // 4
            ):  # Assuming pairs of coordinates and one_hot_trajs and atoms_nums datasets
...
class TrajectoriesDataset_Efficient(Dataset):
    def __init__(
        self, cutoff, scale=1.0, augment=False, dataset=[], original_h5_file=None
    ):
    ...
    def get(self, idx):
        ...
        if frame_idx < self.number_frames - 1:
            ...
            dd, dx, dy, dz = (
                np.zeros_like(coordinates_array),
                np.zeros_like(d_shape),
                np.zeros_like(d_shape),
                np.zeros_like(d_shape),
            )
...
if __name__ == "__main__":
    ...
    TrajsDataset = TrajectoriesDataset_Efficient(
        cutoff, scale, original_h5_file="data/sys_test/resname_unl.h5"
    )
    A, B = TrajsDataset.get(0)
    AA, BB = TrajsDataset[0]
    loader = DataLoader(
        TrajsDataset, batch_size=1, num_workers=8, shuffle=True, pin_memory=False
    )
    for a, b in loader:
        #     print('a')
        #     # del(a)
